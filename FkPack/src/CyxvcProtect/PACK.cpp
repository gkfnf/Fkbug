...
BOOL CPACK::Pack(CString strFilePath, BOOL bSelect[5], CHAR MachineCode[16])
{
	...
	// 获取 TLS 信息
	//BOOL bTLSUseful = objPE.DealWithTLS(pstcShellData);
	...
	*/
	...
	IMAGE_DOS_HEADER* pDos = (IMAGE_DOS_HEADER*)pFinalBuf;
	IMAGE_NT_HEADERS* pNt = (IMAGE_NT_HEADERS*)(pDos->e_lfanew + pFinalBuf);
	IMAGE_FILE_HEADER pFileHeader = (IMAGE_FILE_HEADER)pNt->FileHeader;
	IMAGE_OPTIONAL_HEADER32 pOptionalHeader = (IMAGE_OPTIONAL_HEADER32)(pNt->OptionalHeader);
    
	PIMAGE_SECTION_HEADER pSectionHeader = IMAGE_FIRST_SECTION(pNt);

	// 设置 TLS
	//objPE.SetTLS(pSectionHeader[pFileHeader.NumberOfSections-1].VirtualAddress, (PCHAR)hShell, pstcShellData);
...
BOOL CPACK::SaveFinalFile(LPBYTE pFinalBuf, DWORD pFinalBufSize, CString strFilePath)
{
	...
	wcscat_s(strOutputPath, MAX_PATH, L"_FKBUG");
